...
MASKRCNN_CACHE_FRAMES = 5  # Reuse mask for N frames (4x speedup)
...
with SuppressStdErr():
    ...
    if ENABLE_MASKRCNN_CACHE:
        ...
        print(f"✓ Mask R-CNN caching enabled: reusing mask for {MASKRCNN_CACHE_FRAMES} frames")
    else:
        mask_cache = None
    
    # Initialize advanced metric detectors
    ...
    print("✓ Advanced metric detectors initialized")
...
csv_fields += ["L_draw_detected", "L_draw_time", "R_draw_detected", "R_draw_time"]
...
with SuppressStdErr():  # suppress any backend warnings during loop
    while cap.isOpened():
        ...
        if mask_cache is not None:
            ...
            body_mask = mask_cache.get_or_compute(
                frame_idx, 
                compute_body_mask,
                maskrcnn,
                frame,
        ...
        for side, wrist_idx, detector in [
            ('L', 9, left_recoil_detector),
            ('R', 10, right_recoil_detector)
        ]:
            if wrist_idx in pts:
                recoil_metrics = detector.update(pts[wrist_idx], frame_idx)
                row[f"{side}_recoil_detected"] = int(recoil_metrics['recoil_detected'])
                row[f"{side}_recoil_peak_accel"] = float(recoil_metrics['peak_acceleration'])
                row[f"{side}_recoil_recovery_time"] = int(recoil_metrics['recovery_time_frames'])
            else:
                row[f"{side}_recoil_detected"] = 0
                row[f"{side}_recoil_peak_accel"] = 0.0
                row[f"{side}_recoil_recovery_time"] = 0
        
        # Draw detection
        for side, detector in [
            ('L', left_draw_detector),
            ('R', right_draw_detector)
        ]:
            arm_ext = row[f"{side}_arm_extension"]
